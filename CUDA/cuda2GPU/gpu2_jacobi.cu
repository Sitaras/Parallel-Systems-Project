...
extern "C" double one_jacobi_iteration(int ID, int *devid,double xStart, double yStart,
                           int maxXCount, int maxYCount,
                           double *src, double *dst,
                           double deltaX, double deltaY,
        ...
        if(ID==0){
          err =cudaMemcpy(dst, d_dst, (maxXCount)*(maxYCount-1)*sizeof(double), cudaMemcpyDeviceToHost);
        }else{
          err =cudaMemcpy(dst+maxXCount, d_dst+maxXCount, (maxXCount)*(maxYCount-1)*sizeof(double), cudaMemcpyDeviceToHost);
        }
        if (err != cudaSuccess){
                fprintf(stderr, "GPUassert: %s\n",err);
                return err;
